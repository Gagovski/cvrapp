...
public class UnLockFileCommand extends Command {
    private String mItem;
    private FileListener mListener;
    private ByteBuffer mPayload;

    public UnLockFileCommand(Cvr cvr, FileListener listener, String item) {
        super(cvr);
        mListener = listener;
        mItem = item;
    ...
    @Override
    public short code() {
        return CvrConstants.ResponseCode.UnLockFile;
    }

    @Override
    public void encodeCommand(ByteBuffer b) {
        encodeCommand(b, CvrConstants.CommandCode.UnLockFile, 0, 0, mItem.length() + 1);
    }
