...
public class CapturePicCommand extends Command{
    private MediaListener mListener;

    public CapturePicCommand(Cvr camera, MediaListener listener) {
        super(camera);
        mListener = listener;
    }
    ...
    public void encodeCommand(ByteBuffer b) {
        encodeCommand(b, CvrConstants.CommandCode.CapturePic);
    }
    ...
    @Override
    public short code() {
        return CvrConstants.ResponseCode.CapturePic;
    }

    @Override
    protected void decodeData(short code, int arg1) {
        if (code == CvrConstants.ResponseCode.CapturePic) {
            mListener.onPreviewTakePhoto();
        }
    }
}
